...
const getAllUrls = async () => {
    return new Promise(async (resolve, reject) => {
        ...
        pool.query(
            // Select only not null or empty urls, group by url and join keys and owners with array_agg into the same array
            `SELECT url, array_agg(key) as keys, array_agg(owner_address) as owners FROM signed_texts WHERE url IS NOT NULL AND url != '' GROUP BY url`,
            async (err, res) => {
                if (err) {
                    reject(err);
                } else {
